...
Solver::Solver(std::weak_ptr<rclcpp::Node> n) : node_(n) {
  ...
  angular_velocity_factor_ = node->declare_parameter("solver.angular_velocity_factor", 1.62);
  velocity_factor_ = node->declare_parameter("solver.velocity_factor", 1.5);
...
rm_interfaces::msg::GimbalCmd Solver::solve(const rm_interfaces::msg::Target &target,
  ...
  target_position.z() += dt * target.velocity.z*velocity_factor_;
  //std::cout<<"v_yaw"<<target.v_yaw<<std::endl;
  target_yaw += dt * target.v_yaw*Relative_yaw_angle_deviation_;//角速度比例系数
  ...
  if (target.armors_num == 4){
    ...
    if(std::fabs(target.v_yaw)>8){
      ...
      if(std::fabs(chosen_armor_position.yaw-rpy_[2])<Relative_yaw_angle_deviation_){
