...
ArmorPoseEstimator::ArmorPoseEstimator(
  ...
  // Setup pnp solver
  pnp_solver_ = std::make_unique<fyt::PnPSolver>(camera_info->k, camera_info->d);
...
ArmorPoseEstimator::extractArmorPoses(const ArmorObject & armor,Eigen::Matrix3d R_imu_camera) {//处理单个装甲板目标，填充成ros消息后返回
  ...
  if (pnp_solver_->solvePnPGeneric(
    ...
    Eigen::Vector3d t = fyt::utils::cvToEigen(tvecs[0]);
    ...
    armor_msg.number = kArmorNames[static_cast<int>(armor.label)];
...
void ArmorPoseEstimator::sortPnPResult(const ArmorObject &armor,
  ...
  // 转换为Eigen矩阵
  Eigen::Matrix3d R1 = fyt::utils::cvToEigen(R1_cv);
  Eigen::Matrix3d R2 = fyt::utils::cvToEigen(R2_cv);
  ...
  // 计算灯条在图像中的倾斜角度
  double l_angle =
      std::atan2(armor.landmarks[1]-armor.landmarks[3], armor.landmarks[0]-armor.landmarks[2]) * 180 / M_PI;// dy/dx
  double r_angle =
      std::atan2(armor.landmarks[7]-armor.landmarks[5], armor.landmarks[6]-armor.landmarks[4]) * 180 / M_PI;
  double angle = (l_angle + r_angle) / 2;
  angle += 90.0;
  // std::cout<<l_angle<<std::endl;
  // std::cout<<r_angle<<std::endl;
  if (armor.label == 6) angle = -angle;
  ...
  if ((angle > 0 && rpy1[2] > 0 && rpy2[2] < 0) ||
    ...
    std::cout<<"openvino_armor_detector:PnP Solution 2 Selected"<<std::endl;
  }
}
